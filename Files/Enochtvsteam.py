...
class streamer:
    ...
    def resolve(self, url):
        try:
            ...
            if '/play/tt' in url: u = self.ratu21(url)
...
#Link provider and quality            
    ...
    def ratu21(self, url):
        try:
            u = client.request(url)
            e = re.findall('JuicyCodes.Run\("(.+?)"\)', u)
            e = [i.replace('"+"',' ') for i in e]
            e = base64.b64decode(e[0])
            e = jsunpack.unpack(e)
            e = re.findall('"file":"([^"]*)","label":"([^"]*)', e)
            e = [(i[0],i[1]) for i in e if 'api' in i[0]]
            return e
        except:
            return